...
def _converter_tab() -> str:
    ...
    tools = _p(
        "Czy Pandoc / Calibre są dostępne w tym środowisku — sprawdzisz w <b>dolnym pasku "
        "statusu</b> (np. " + _code("Pandoc: OK") + ", " + _code("Calibre: brak") + ")."
    )
    return _section("Konwerter → EPUB", intro) + formats + options + engine + drm + tools
...
def _fixer_tab() -> str:
    ...
    return _section("Fixer (naprawa + CSS)", intro + hyphen + css + preset)
...
def _validation_tab() -> str:
    ...
    status = _p(
        "Czy Java i EpubCheck są wykryte <b>teraz</b> — sprawdzisz w <b>dolnym pasku statusu</b> "
        "(" + _code("Java: OK/brak") + " | " + _code("EpubCheck: OK/brak") + ")."
    )
    return _section("Walidacja (EpubCheck)", intro + java) + levels + click + status
