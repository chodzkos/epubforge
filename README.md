...
| Metadane Dublin Core (+ seria/tom/liczba stron EPUB 3) | ✅ |
...
- **📖 Symulator czytnika** — neutralne profile reflowable (scroll/strony CSS columns), osobny tor skalowania fixed-layout, jawna warstwa ustawień użytkownika, porównanie dwóch viewportów, screenshot oraz diagnostyka layoutu prowadząca do elementu źródłowego. Nazwy profili nie sugerują emulacji silnika konkretnego urządzenia.
- **✅ Walidacja EpubCheck** — raport błędów/ostrzeżeń EPUB; dwuklik błędu skacze do linii w edytorze, a górny pasek otwiera zaznaczony lub ostatnio zwalidowany cały EPUB w Sigilu/Calibre Editor (wymaga Javy + epubcheck.jar)
...
- **🏷️ Metadata** — pełna edycja Dublin Core + seria/tom (Calibre i EPUB 3) oraz widoczna, edytowalna **liczba stron** w EPUB 3. Wartość można wpisać ręcznie, pobrać z katalogu albo oszacować przez istniejący moduł Statystyki (domyślnie 250 słów/stronę); wyczyszczenie pola usuwa `schema:numberOfPages`. Liczba z katalogu opisuje wydanie papierowe, estymacja opisuje objętość tekstu, a „strony podglądu” symulatora są technicznym wynikiem viewportu — to trzy różne pojęcia. EPUB 2 wymaga konwersji do EPUB 3 przed zapisem tego pola. Przycisk **Pobierz metadane…** wciąga po ISBN wydawcę, rok, liczbę stron i deskryptory przedmiotowe z Biblioteki Narodowej → LubimyCzytac → Open Library → Google Books (wybór pól do nadpisania, nigdy ciche). Uwaga: e-booki mają **własny ISBN wydania elektronicznego**, którego katalog BN (głównie wydania papierowe) zwykle nie ma — przy pudle na ISBN aplikacja automatycznie **dopasowuje po tytule** i oznacza to w wyniku („dopasowanie po tytule"); ISBN pliku nie jest przy tym nadpisywany. Bez ISBN — wyszukiwanie po **tytule/autorze** w LubimyCzytac z listą kandydatów (opisy, cykle, kategorie; cache + rate limiter dla grzecznościowego scrapingu)
